...
def licenses_bin(top, arch, output_file = nil, bsp = nil, tc = nil, cur_release = nil)
    ...
    licenses = CLicenseCollection.new("MEPA", license_options)
    licenses.concat(CLicenseModuleInfo.new(top, "MEPA"))

    licenses.concat(CLicenseCsv.new([bsp[:path]], "BSP")) if bsp
    licenses.concat(CLicenseCsv.new([tc[:path]], "Toolchain")) if tc

    license_options[:hdr_pre]  = "This file provides a summary of the licenses included in the binary MEPA\n"
    ...
    license_options[:hdr_post] += "- MEPA:      Contact sales/marketing and request the source code.\n"
